						...
						void *response)
{
	...
	} else if (0 == strncmp(VIRTUAL_ROOT_PATH(context), parent_path, strlen(VIRTUAL_ROOT_PATH(context)) - 1)) {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), folder_name, NULL);
	} else {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), parent_path, "/", folder_name, NULL);
	}	
						...
						void *response)
{
	...
	} else if (0 == strncmp(VIRTUAL_ROOT_PATH(context), dir_path, strlen(VIRTUAL_ROOT_PATH(context)) - 1)) {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), file_name, NULL);
	} else {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), dir_path, "/", file_name, NULL);
	}
						...
						void *response)
{
	...
	if (NULL == parent_path) {
	        ...
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), parent_path, "/", file_name, NULL);
	}	
	storage_plugin_sample_debug("Remove path (%s)", path);
	int ret = remove(path);
	storage_plugin_sample_debug("Remove ret (%d)", ret);
						...
						void *response)
{
	...
	if (NULL == parent_path) {
	        ...
	        old_path = g_strconcat(VIRTUAL_ROOT_PATH(context), parent_path, "/", file_name, NULL);
	}
	storage_plugin_sample_debug("Old path (%s)", old_path);

	if (NULL == dest_parent_path) {
	        new_path = g_strconcat(VIRTUAL_ROOT_PATH(context), "/", new_file_name, NULL);
	} else if (0 == strncmp(VIRTUAL_ROOT_PATH(context), dest_parent_path, strlen(VIRTUAL_ROOT_PATH(context)) - 1)) {
	        new_path = g_strconcat(VIRTUAL_ROOT_PATH(context), new_file_name, NULL);
	} else {
	        new_path = g_strconcat(VIRTUAL_ROOT_PATH(context), dest_parent_path, "/", new_file_name, NULL);
	}
						...
						void *response)
{
	...
	if (NULL == parent_path) {
	        ...
	        old_path = g_strconcat(VIRTUAL_ROOT_PATH(context), parent_path, "/", folder_name, NULL);
	}
	storage_plugin_sample_debug("Old path (%s)", old_path);
	...
	if (NULL == dest_parent_path) {
	        ...
	        new_path = g_strconcat(VIRTUAL_ROOT_PATH(context), dest_parent_path, "/", new_folder_name, NULL);
	}
	storage_plugin_sample_debug("New path (%s)", new_path);
						...
						void *res)
{
	...
	if (NULL == folder_path) {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), "/", file_name, NULL);
	} else if (0 == strncmp(VIRTUAL_ROOT_PATH(context), folder_path, strlen(VIRTUAL_ROOT_PATH(context)) - 1)) {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), file_name, NULL);
	} else {
	        path = g_strconcat(VIRTUAL_ROOT_PATH(context), folder_path, "/", file_name, NULL);
	}
...
EXPORT_API storage_adaptor_plugin_handle_h create_plugin_handle(void)
{
	...
	/* Optional functions */
	/* Unified Storage System internal */
