#ifndef __AUTH_PLUGIN_ESU_LOG_H__
#define __AUTH_PLUGIN_ESU_LOG_H__
 ...
 *
 *    # dlogutil AUTH_PLUGIN_ESU:D
 *
 *  You may use different logging levels as: D (debug), I (info), W (warning), E (error) or F (fatal).
 *  Higher level messages are included by default e.g. dlogutil AUTH_PLUGIN_ESU:W prints warnings but also errors and fatal messages.
 */
...
#define LOG_TAG "AUTH_PLUGIN_SAMPLE"
...
#define FONT_COLOR_BOLDWHITE  "\033[1m\033[37m"      /* Bold White */
	 ...
	 *  Gets thread ID
	 */
#define auth_plugin_sample_gettid() syscall(__NR_gettid)
 ...
 */
#define auth_plugin_sample_debug(fmt, arg...) do { \
	LOGD(FONT_COLOR_GREEN"[%d]"fmt""FONT_COLOR_RESET, auth_plugin_sample_gettid(), ##arg);     \
} while (0)
 ...
 * @param[in]  args Arguments to be displayed
 */
#define auth_plugin_sample_warning(fmt, arg...) do { \
	LOGW(FONT_COLOR_YELLOW"[%d]"fmt""FONT_COLOR_RESET, auth_plugin_sample_gettid(), ##arg);     \
} while (0)
 ...
 * @param[in]  args Arguments to be displayed
 */
#define auth_plugin_sample_error(fmt, arg...) do { \
	LOGE(FONT_COLOR_RED"[%d]"fmt""FONT_COLOR_RESET, auth_plugin_sample_gettid(), ##arg);     \
} while (0)
 ...
 * @param[in]  args Arguments to be displayed
 */
#define auth_plugin_sample_fatal(fmt, arg...) do { \
	LOGF(FONT_COLOR_BOLDRED"[%d]"fmt""FONT_COLOR_RESET, auth_plugin_sample_gettid(), ##arg);     \
} while (0)
 ...
 * @param[in]  args Arguments to be displayed
 */
#define auth_plugin_sample_debug_func(fmt, arg...) do { \
	LOGD(FONT_COLOR_CYAN"[%d]"fmt""FONT_COLOR_RESET, auth_plugin_sample_gettid(), ##arg);     \
} while (0)
...
#endif /* __AUTH_PLUGIN_ESU_LOG_H__ */
